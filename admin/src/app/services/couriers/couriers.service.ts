import { Courier } from 'src/app/interfaces/courier';
...
export class CouriersService {
    ...
    public async add(params: any) {
        const response = await this.api.post(environment.store, '/store/couriers/add', params);

        if (response.ok) {
            params.courierId = response.result.courierId;
    ...
    public async update(params: any) {
        const response = await this.api.post(environment.store, '/store/couriers/update', params);

        if (response.ok) {
            for (let i = 0; i < this.data.length; i++) {
                if (this.data[i].courierId == params.courierId) {
    ...
    public async delete(params: any) {
        const response = await this.api.post(environment.store, '/store/couriers/delete', params);
    ...
    public async unsubscribe(params: any) {
        ...
        const response = await this.api.post(environment.store, '/store/couriers/unsubscribe', params);
