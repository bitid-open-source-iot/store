...
export class SuppliersPage implements OnInit, OnDestroy {
    ...
    constructor(public menu: MenuService, private toast: ToastService, private sheet: MatBottomSheet, private dialog: MatDialog, public suppliers: SuppliersService, private route: ActivatedRoute, private router: Router) { };
    ...
    private async list() {
        ...
        const response = await this.suppliers.list({
            'filter': [
                'role',
                'icon',
                'storeId',
        ...
        if (response.ok) {
            this.suppliers.data = response.result;
        } else {
            this.suppliers.data = [];
        };
                    ...
                    case (3):
                        ...
                        await share.afterClosed().subscribe(async result => {
                            if (result) {
                                ...
                                const response = await this.suppliers.share({
                                    'role': result.role,
                                    'email': result.email,
                                    'storeId': this.storeId,
                                    'supplierId': supplier.supplierId
                    ...
                    case (5):
                        ...
                        await unsubscribe.afterClosed().subscribe(async result => {
                            if (result) {
                                ...
                                const response = await this.suppliers.unsubscribe({
                                    'storeId': this.storeId,
                                    'supplierId': supplier.supplierId
                                });

                                if (response.ok) {
                                    this.toast.success('you were unsubscribed from supplier!');
                                } else {
                                    this.toast.error(response.error.message);
                                };
                    ...
                    case (6):
                        ...
                        await remove.afterClosed().subscribe(async result => {
                            if (result) {
                                ...
                                const response = await this.suppliers.delete({
                                    'storeId': this.storeId,
                                    'supplierId': supplier.supplierId
                                });

                                if (response.ok) {
                                    this.toast.success('supplier was deleted!');
                                } else {
                                    this.toast.error(response.error.message);
                                };
